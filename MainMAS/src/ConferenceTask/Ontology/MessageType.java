package ConferenceTask.Ontology;

/**
 * Created with IntelliJ IDEA.
 * User: Andrew
 * Date: 17.01.14
 * Time: 22:17
 * To change this template use File | Settings | File Templates.
 */
public interface MessageType
{
    public static final String YOU_ARE_BOSS = "You are boss";
    public static final String I_AM_BOSS = "I am boss";
    public static final String I_AM_NEW = "I am new";
    public static final String SCHEDULE = "Schedule";
    public static final String IT_IS_GOOD_SCHEDULE = "It is good schedule";
    public static final String ALTERNATIVE_SCHEDULE = "I propose alternative schedule";
    public static final String VOTING = "Voting";
    public static final String ACCEPT_AGENT = "Accept agent";
    public static final String REJECT_AGENT = "Reject";
    public static final String VOTE_YES = "YES";
    public static final String VOTE_NO = "NO";
    public static final String FINISH = "Finish";
    public static final String SAY_RATING = "Say your rating";
    public static final String RATING_OF_SCHEDULE = "rating of schedule in attachment";
    public static final String NEW_ROUND = "New round";
}
